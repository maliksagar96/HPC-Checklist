...
__global__ void gpuHistogram(int *input, int *hist, int size, int bins) {
	...
	//load from gloabal memory to shared memory
	if(tx < bins) {
		localHist[tx] = 0;
	}
	...
	if(tid < size) {
		atomicAdd(&localHist[input[tid]%bins], 1);
	}

	__syncthreads();

	if(tx < bins) {
		atomicAdd(&hist[tx], localHist[tx]);
	}
...
int main() {
	...
	gpuHistogram<<<grid, block, binsByteSize>>>(d_input, d_hist, size, bins);
